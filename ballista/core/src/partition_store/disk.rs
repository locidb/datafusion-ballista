...
impl PartitionStore for DiskBasedPartitionStore {
    fn store_batch(&self, path: &str, batch: RecordBatch) -> Result<(), BallistaError> {
        log::debug!("DiskBasedPartitionStore.store_batch: {}", path);
    ...
    fn finalize_batches(&self, path: &str) -> Result<(), BallistaError> {
        log::debug!("DiskBasedPartitionStore.finalize_batches: {}", path);
        let mut batch_writers = self.batch_writers.lock().unwrap();
    ...
    async fn store_partition(
        ...
        log::debug!("DiskBasedPartitionStore.store_partition: {}", path);
        let file = File::create(path).map_err(|e| {
            error!("Failed to create partition file at {}: {:?}", path, e);
            BallistaError::IoError(e)
        })?;
    ...
    fn fetch_partition(
        ...
        log::debug!("DiskBasedPartitionStore.fetch_partition: {}", path);
    ...
    fn delete_partition(&self, path: &str) -> Result<(), BallistaError> {
        log::debug!("DiskBasedPartitionStore.delete_partition: {}", path);
    ...
    fn take_partition(
        ...
        log::debug!("DiskBasedPartitionStore.take_partition: {}", path);
